...
public abstract class WebSocket07FrameSinkChannel extends StreamSinkFrameChannel {
    ...
    protected WebSocket07FrameSinkChannel(WebSocket07Channel wsChannel, WebSocketFrameType type,
        ...
        if (wsChannel.hasNegotiatedExtensions() &&
            ...
            List<ExtensionFunction> negotiatedExtensions = wsChannel.getNegotiatedExtensions();
            for (ExtensionFunction extension : negotiatedExtensions) {
                rsv = extension.writeRsv(rsv);
            }
        }
        setRsv(rsv);
    }
